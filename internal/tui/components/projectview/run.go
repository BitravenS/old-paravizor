...
func (m *Model) startRun() tea.Cmd {
	...
	cfg := *m.ctx.Config
	var projectCfg *proj.ProjectConfig
	if m.projCfg != nil {
		copyCfg := *m.projCfg
		projectCfg = &copyCfg
	}
	...
	if projectCfg != nil {
		includeTargets = append(includeTargets, projectCfg.Scope.Include...)
	}
	...
	go func() {
		doneCh <- runtime.RunPipeline(ctx, runtime.Options{
...
func (m *Model) applyEvent(e events.Event) {
	...
	case events.LogMessage:
		m.appendLog(fmt.Sprintf("[%s] %s: %s", ts, v.Level, v.Message))
