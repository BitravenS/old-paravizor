...
type NodeConfig struct {
	...
	Name      string        `yaml:"name" validate:"required"`
	Stage     int           `yaml:"stage" validate:"gte=0"`
...
type RouteConfig struct {
	...
	Condition string `yaml:"condition,omitempty" validate:"omitempty,regex"` // Optional condition to determine if results should be routed (e.g., based on output content)
}
